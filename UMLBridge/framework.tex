...
\begin{figure}[htbp]
	\centering
		\includegraphics[width=0.80\textwidth]{figures/overall.JPG}
...
At the metamodeling level the bridge takes as input the UML metamodel and the profile. As shown in the figure \ref{fig:overall}, it generates a metamodel by coping the UML metamodel in ECORE formalism, and by adding futher constructs which represent the elements in the profile.  The figure \ref{fig:overall} represents a UML profile translated into an EMF metamodel. All the UML metamodel elements are in the target metamodel, all the profiled elements (stereotypes and further data types) are represented. The extension mechanism of a stereotype and its metaclass is represented by means of a binding mechanism. Every super class has a reference to the eventual stereotype.
\begin{figure}[htbp]
	\centering
...
\begin{figure}[htbp]
	\centering
		\includegraphics[width=1.00\textwidth]{figures/modelingExample.jpg}
