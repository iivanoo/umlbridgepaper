...
In this case study we use the Papyrus modeling tool\footnote{Papyrus UML web site: \small{\url{http://www.papyrusuml.org}}} for
...
\textbf{Step 5.} In order to impartially test the bridge, we decided to reuse an already existing SysML model. 
...
\begin{figure}
  \centering
  \subfloat[]{\label{fig:hsuvUML}\includegraphics[scale=0.42]{figures/hsuvUML.png}}
 \hspace{2mm}
  \subfloat[]{\label{fig:hsuvMOF}\includegraphics[scale=0.42]{figures/hsuvMOF.png}}
...
More specifically, the manipulation tool is implemented as an ATL transformation that \ivano{completare...}.
%
...
does not require to use UML-specific constructs (lines \ivano{XXX} and \ivano{YYY} in Listing \ref{lst:manipulationTool} 
show the ATL instructions needed if the input UML model was not bridged).  


\ivano{lessons learned}
