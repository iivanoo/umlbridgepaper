...
\textbf{Annotation models}. 
%Since the slicing mechanism is realized as a model transformation, 
...
%For example, if designers already know they will use only specific UML diagrams, then the second option  may be the most convenient;
%or else, if designers already defined some initial model, they may use the third option.
...
$Tslicer$ 
%automatically adapts the input transformation according to the meta-elements (i.e., metaclasses, attributes, references) that were previously sliced out from $MM_{sliced}$.
